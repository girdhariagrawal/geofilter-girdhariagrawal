...
describe('Testing the With in Range feature', () => {
  const center = {
    latitude: 53.339428,
    longitude: -6.257664,
  };
  it('Distance between two points in kms', () => {
    assert.equal(
  ...
  it('Distance with same coordinates records', () => {
    assert.equal(
      distanceBetweenPoints(
        center.latitude,
        center.longitude,
  ...
  it('Distance between two points in miles', () => {
    assert.equal(
      distanceBetweenPoints(
        center.latitude,
        center.longitude,
  ...
  it('With empty records', () => {
    assert.deepEqual(getDataWithInRange([], center, 20), []);
  });
  it('With empty records', () => {
    assert.deepEqual(getDataWithInRange([], center, 20), []);
  });
  it('With in 20kms, Should return one result set', () => {
    assert.deepEqual(getDataWithInRange(records, center, 20), [{
      latitude: '53.2451022',
      user_id: 4,
      name: 'Ian Kehoe',
      longitude: '-6.238335',
  ...
  it('Success: With in 0kms, Should return empty result set', () => {
    assert.deepEqual(getDataWithInRange(records, center, 0), []);
  });

  it('Success: With in 100kms Should return one result set', () => {
    assert.deepEqual(getDataWithInRange(records, center, 100), [{
      latitude: '52.986375',
      user_id: 12,
